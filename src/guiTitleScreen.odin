...
draw_title_screen :: proc() {
	...
	if (GuiButton(
			   {state.cursor.x, state.cursor.y, TITLE_BUTTON_WIDTH, TITLE_BUTTON_HEIGHT},
			   "New Combat",
		   )) {
	...
	//MyGuiTextInput({state.cursor.x, state.cursor.y, 200, 50}, &state.test_text_input)
	...
	rl.GuiLabel(
		{state.cursor.x, state.cursor.y, 100, 50},
		fmt.ctprintf("v%v.%v", VERSION_MAJOR, VERSION_MINOR),
	)
}
