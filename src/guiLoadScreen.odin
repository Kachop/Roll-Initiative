...
draw_load_screen :: proc() {
    ...
    text_align_center()
    GuiLabel({state.cursor.x, state.cursor.y, state.window_width - (state.cursor.x * 2), MENU_BUTTON_HEIGHT}, "Load Combat")
    ...
    rl.GuiLine({state.cursor.x, state.cursor.y, state.window_width - PADDING_LEFT - PADDING_RIGHT, LINE_WIDTH}, "")
    state.cursor.y += LINE_WIDTH + MENU_BUTTON_PADDING

    if (GuiFileDialog({state.cursor.x, state.cursor.y, state.window_width - PADDING_LEFT - PADDING_RIGHT, state.window_height - state.cursor.y - PADDING_BOTTOM})) {
        combat, ok := load_combat_file(str(state.load_screen_state.selected_file))

        if ok {
            state.current_combat = combat
