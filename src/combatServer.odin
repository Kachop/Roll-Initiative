...
get_ip_windows :: proc() -> (ip_string: string, err: os2.Error) {
	...
	ip_string = fmt.aprintf(
		"%v.%v.%v.%v",
		state.config.IP_ADDRESS[0],
		state.config.IP_ADDRESS[1],
		state.config.IP_ADDRESS[2],
		state.config.IP_ADDRESS[3],
	)
	return
}
...
get_ip_linux :: proc() -> (ip_string: string, err: os2.Error) {
	...
	p: os2.Process;{
		...
		p = os2.process_start({command = {"ip", "-br", "-j", "a"}, stdout = w}) or_return
	}
...
run_combat_server :: proc() {
	...
	err := http.listen_and_serve(
		&s,
		routed,
		net.Endpoint{address = state.config.IP_ADDRESS, port = state.config.PORT},
		opts,
	)
