...
init_file :: proc(filename: string) -> JSONFile {
	return JSONFile{filename, "", 0}
}

add_int :: proc(name: string, val: Integer, file: ^JSONFile) {
	add_indent(file)
	file.json_string = str(file.json_string, "\"", name, "\": ", val, ",\n", sep = "")
}
...
add_array :: proc(name: string, vals: Array, file: ^JSONFile) {
	add_indent(file)
	if name != "" {
		file.json_string = str(file.json_string, "\"", name, "\": [\n", sep = "")
	} else {
		file.json_string = str(file.json_string, "[\n", sep = "")
	}
	file.indent_level += 1
	...
	for name, val in vals {
		...
		switch t in val {
		case Integer:
			add_int(name, val.(Integer), file)
		case Float:
			add_float(name, val.(Float), file)
		case Boolean:
			add_bool(name, val.(Boolean), file)
		case String:
			add_string(name, val.(String), file)
		case Array:
			add_array(name, val.(Array), file)
		case Object:
			add_object(name, val.(Object), file)
		}
	}
	file.indent_level -= 1
	add_indent(file)
...
add_indent :: proc(file: ^JSONFile) {
	for _ in 0 ..< file.indent_level {
		file.json_string = str(file.json_string, "\t", sep = "")
	}
}
